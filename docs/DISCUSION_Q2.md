...
>
> ✅ **Yun 2014 leído a texto completo (20-ago-2026):** no responde "qué parámetros importan más" (nadie de los seis lo hace, se abandona esa sub-pregunta sin bloquear nada). **El toolbox+datos del material suplementario se confirmaron y se consiguió el manual** — el link de descarga de la universidad está muerto, pero **el de SourceForge sigue vivo**: `sourceforge.net/projects/gaitkinematicsprediction/files/Gait_Kinematics_Prediction_V1.01_Release.zip/download`. Código BSD libre; **la base de datos KIST tiene licencia restringida (solo uso dentro del toolbox, no redistribuir)** — no afecta a P-24 porque la validación ya es contra Camargo, no contra KIST. **Pendiente del usuario:** descargar el `.zip` y confirmar que corre — sería el candidato más fácil de cumplir P-23 (nada que reimplementar).
>

% ============================================================
% !!! SUPERADO POR EL PIVOTE — 19-ago-2026 !!!
% Todo el contenido de este archivo (Introduction, 5.1-5.4, Results,
% Discussion) describe el enfoque ANTERIOR: fidelidad de seguimiento
% multi-sujeto con CSV pregrabado. Ese enfoque quedo reemplazado por
% completo -- ver el banner al inicio de CLAUDE.md (raiz del repo) y
% docs/DISCUSION_Q2.md P-20/P-21. Nuevo enfoque: generacion de
% trayectoria desde antropometria, validada contra bases de datos
% publicas independientes (ver docs/planificacion/
% analisis_escalamiento_Q1_generador_trayectorias.md, plan vigente).
% Se conserva sin editar por trazabilidad -- reescribir antes de usar.
% Revista tambien reabierta, ver docs/manuscrito/guia_autor_JTEHM.md.
% ============================================================
...
\subsection*{Vertical Force Overestimation: A Two-Stage, Decoupled Explanation}
...
overestimation is decomposed into two sequentially corrected sources
rather than a single inertial term: (1) a fixed vertical datum offset,
calibrated by sweeping the trajectory's starting height against an
independent static load reference; and (2) command-to-encoder tracking
fidelity per axis, isolating mechanical/control error from the
remaining source. Because the moving assembly's weight is not
uniformly distributed across its frame, an axis-wise inertial term
computed a priori from a measured or CAD-derived mass was judged
unreliable; instead, the portion of the residual attributable to
inertial effects (beyond what datum compression explains) is inferred
directly from the same height-sweep curve used to calibrate the datum
offset (1), rather than isolated as an independent, a-priori-known
mass term. Both stages are benchmarked against published prosthetic
gait GRF data.
% [RESUELTO 13-ago-2026, ver DISCUSION_Q2.md P-5: m_eje no se mide (ni
% CAD ni pesaje, por distribucion no uniforme del peso del ensamblaje)
% -- se infiere indirectamente del barrido de alturas de offset (1),
% fusionando lo que antes eran items (1) y (3) en un solo ensayo
% empirico. El aislamiento exacto inercia-vs-offset residual queda
% como limitacion declarada, no como separacion perfecta. Sigue
% bloqueado por la integracion Raspberry Pi-ESP32 -- sin datos reales
% todavia.]

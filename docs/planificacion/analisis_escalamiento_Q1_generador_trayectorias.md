...
**Hallazgo nuevo y más valioso que la pregunta original:** Yun 2014 publica, como Material Suplementario en ScienceDirect, un **toolbox de Matlab/Octave ("Gait Kinematics Prediction Toolbox") y la base de datos completa de los 113 sujetos ("KIST Human Gait Pattern Data")** — código y datos ya listos para correr, no solo coeficientes en una tabla. Si está accesible, es potencialmente **más fuerte que Koopman o Zhao como candidato**, porque no hay que reimplementar nada — se ejecuta el toolbox publicado tal cual (cumple P-23 al pie de la letra) y además entrega incertidumbre por punto del ciclo. **Acción pendiente para el usuario:** en la misma sesión de PUCP donde consiguió el PDF, revisar si la página de ScienceDirect tiene un enlace de "Supplementary material" — WebFetch devolvió 403 sin esa sesión autenticada, no se pudo confirmar desde acá.

...
- **Paso 5 (material suplementario del paper Yun 2014, ScienceDirect) — también pendiente, dejado para la próxima sesión en la Mac.** Necesita acceso institucional PUCP (VPN o red de campus) para descargar `mmc1.pdf`/`mmc2.pdf` y colocarlos en `docs/literatura/pdfs/yun2014_supp/`. Sin este paso no se pierde funcionalidad de código — es solo referencia documental para `docs/algoritmo/diseno_matematico_generador.md`. Ver instrucciones completas abajo.

function T6 = Evaluar_Individual_Kuopio_Tobillo_Zhao()
% EVALUAR_INDIVIDUAL_KUOPIO_TOBILLO_ZHAO  26-ago-2026 (tarea nocturna
%                   /loop, Agente A, PLAN_ZHAO_YUN.md Tarea 1): replica
%                   EXACTA de Evaluar_Individual_Kuopio_Tobillo.m para el
%                   candidato ZHAO 2026 - misma metodologia y mismos 6
%                   sujetos, solo cambia la fuente del calculo (llama a
%                   Evaluar_vs_Kuopio_Tobillo_Fases_Zhao en vez de la
%                   version de Koopman).
%
%   UNA SOLA IMPLEMENTACION DEL MODELO: este script NO recalcula nada -
%   llama a Evaluar_vs_Kuopio_Tobillo_Fases_Zhao(false) y grafica las
%   mismas curvas que producen las estadisticas reportadas.
%
%   MISMOS 6 sujetos que Koopman/RODILLA/INCLINACION_TIBIAL (el mas pesado
%   40, el mas alto 37, el mas liviano 43, la mas baja 46 F, una mujer
%   media 19, un hombre de talla media 28) - deliberadamente los mismos,
...
[~, D] = Evaluar_vs_Kuopio_Tobillo_Fases_Zhao(false);
pct = D.pct;

fig = figure('Name','TOBILLO Zhao - comparacion individual, Kuopio', 'Position',[30 20 1400 1000], 'Color','w');
...
for r = 1:nfil
    ...
    if r == 1, legend({'real','modelo Zhao'}, 'Location','northwest'); end
...
fprintf('\n=== TOBILLO Zhao, comparacion individual (6 sujetos de antropometria diversa) ===\n');
disp(T6);

sgtitle('TOBILLO Zhao - sujeto por sujeto (cada uno con SU sexo/talla/masa y SU velocidad medida)', 'FontWeight','bold');

writetable(T6, fullfile(carpeta, 'Evaluar_Individual_Kuopio_Tobillo_Zhao_resultados.csv'));
out_png = fullfile(carpeta, 'Evaluar_Individual_Kuopio_Tobillo_Zhao_figura.png');
exportgraphics(fig, out_png, 'Resolution', 150);
fprintf('Figura guardada en: %s\n', out_png);
fprintf('Tabla guardada en: %s\n', fullfile(carpeta, 'Evaluar_Individual_Kuopio_Tobillo_Zhao_resultados.csv'));

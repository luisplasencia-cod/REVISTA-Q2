function [T, D] = Evaluar_vs_Kuopio_Tobillo_Fases_Zhao(hacer_figura)
% EVALUAR_VS_KUOPIO_TOBILLO_FASES_ZHAO  27-ago-2026: replica EXACTA del
...
%                   Zhao 2026 en vez de Koopman 2014. Unico cambio de
%                   fondo: la fuente de theta_muslo/theta_tibia pasa de
%                   Koopman2014_Core (via Obtener_Angulos_Candidato
%                   'Koopman') a Zhao2026_Core (via Obtener_Angulos_
%                   Candidato 'Zhao', lado nativo 'izquierda' por
%                   defecto - D2 de DECISIONES.md, NO se usa el "truco de
%                   lado" 'derecha'). Cadena_Completa_Core, el residuo de
%                   rockers LOSO, el cierre de ciclo/zancada y el formato
%                   de figura/tabla son CANDIDATO-AGNOSTICOS y se dejan
%                   sin cambios de diseno, tal como exige el plan.
%
%   IMPORTANTE: la RODILLA no se toca aqui (igual que en Koopman) - ver
%   CIERRE_TOBILLO.md #6. Este archivo NO recalcula rodilla.
%
%   MAPEO - de donde viene cada pieza (pedido explicito del usuario,
%   25-ago-2026, para trazabilidad en el articulo):
%
%   1) DE ZHAO 2026 (publicado, sin modificar, lado nativo 'izquierda'):
%      phi_cadera(t), theta_tibia(t)=phi_cadera-phi_rodilla (Zhao2026_Core.m,
%      via Obtener_Angulos_Candidato('Zhao',...) - ver PLAN_ZHAO_YUN.md)
%
...
%      se usa para ajustar los coeficientes de Zhao ni la logica de
%      Cadena_Completa_Core.m, que ya estaban fijados antes de esto.
%
% RESULTADO (Zhao, 27-ago-2026): ver Evaluar_vs_Kuopio_Tobillo_Fases_Zhao_
% resultados.csv y BITACORA_NOCHE.md para los numeros reales de esta
% corrida - no se copian a mano los de Koopman aqui.
%
...
% --- Paso 1a: cargar sujetos, angulos del candidato (Zhao) y angulos REALES ---
...
for i = 1:n
    ...
    try
        ...
        tempo = Temporizacion_Core(antro, 'Zhao');
        [th_m, th_t, tempo] = Obtener_Angulos_Candidato('Zhao', antro, tempo, npts);
...
% Mismo metodo que Koopman (ver cabecera): la correccion se aplica DONDE
% esta el error - el angulo publicado por el candidato - y no sobre la
% posicion ya construida.
...
writetable(T, fullfile(carpeta, 'Evaluar_vs_Kuopio_Tobillo_Fases_Zhao_resultados.csv'));
fprintf('Tabla: %s\n', fullfile(carpeta, 'Evaluar_vs_Kuopio_Tobillo_Fases_Zhao_resultados.csv'));
...
out_png = fullfile(carpeta, 'Evaluar_vs_Kuopio_Tobillo_Fases_Zhao_figura.png');

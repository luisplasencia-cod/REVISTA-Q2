function [T, D] = Evaluar_vs_Kuopio_Tobillo_Fases(hacer_figura)
% EVALUAR_VS_KUOPIO_TOBILLO_FASES  25-ago-2026: MODELO FINAL de tobillo -
%                   fases + residuo empirico de rockers (LOSO). Reemplaza
%                   a Evaluar_vs_Kuopio_Tobillo.m (formula continua +
%                   restriccion cummax) como version final de tobillo.
%
%   IMPORTANTE: la RODILLA no se toca aqui - se probo el modelo por fases
%   tambien para rodilla y la EMPEORO (r_y bajo de 0.892 a 0.689, con una
%   subida irreal de +4.8cm en apoyo que el dato real no tiene - ver
%   CIERRE_TOBILLO.md #6). La rodilla se queda con el modelo continuo de
%   siempre (RODILLA/Kuopio/Evaluar_vs_Kuopio_Avance.m, sin cambios,
%   r_x=0.996, r_y=0.892) - este archivo NO recalcula rodilla.
%
%   MAPEO - de donde viene cada pieza (pedido explicito del usuario,
%   25-ago-2026, para trazabilidad en el articulo):
%
%   1) DE KOOPMAN 2014 (publicado, sin modificar):
%      theta_cadera(t), theta_tibia_via_rodilla(t) (Koopman2014_Core.m)
%
...
%      se usa para ajustar los coeficientes de Koopman ni la logica de
%      Cadena_Completa_Core.m, que ya estaban fijados antes de esto.
%
% RESULTADO: X r=0.998 (RMSE=2.90cm), Y r=0.985 (RMSE=1.54cm), N=15.
% Historial: formula continua + cummax -> X r=0.981, Y r=0.754; fases +
% rockers + ganancia de posicion -> X r=0.997/4.46cm, Y r=0.953/1.99cm;
% angulos calibrados + cierre de zancada (actual) -> lo de arriba.
%
% CAMBIO DE FONDO 25-ago-2026 - la correccion ya NO se aplica sobre la
% posicion sino sobre el ANGULO: Koopman reproduce la forma del ciclo casi
% exacta pero sobreestima la excursion angular ~20-23% en esta poblacion
% (ganancia LOSO: muslo 0.769, tibia 0.811 - la de tibia se habia
% encontrado antes, por separado, en INCLINACION_TIBIAL/, lo que confirma
% que es UN defecto del modelo publicado y no uno por eje). Calibrando los
% dos angulos con una afin LOSO y propagando por la geometria, la
% calibracion de ganancia por eje sobre X/Y deja de hacer falta y las 4
% metricas mejoran a la vez.
%
%   SALIDAS
...
%         Evaluar_Individual_Kuopio_Tobillo.m para que exista UNA SOLA
...
% --- Paso 1a: cargar sujetos, angulos de Koopman y angulos REALES ---
Sarr = cell(n,1); Antro = cell(n,1); Tempo = cell(n,1);
Thm_s = cell(n,1); Tht_s = cell(n,1); pct_nat_arr = cell(n,1);
Thm_koop = nan(n,101); Tht_koop = nan(n,101);   % en malla 0:100, grados
...
% Este es el cambio de fondo del 25-ago-2026 (ver cabecera): la correccion
% se aplica DONDE esta el error - el angulo publicado por Koopman, que
% sobreestima la excursion ~20-23% - y no sobre la posicion ya construida.
...
for k = 1:numel(idx_ok)
    ...
    % Efecto medido: RMSE de X baja de 4.08 a 2.90cm, r sin cambio (0.998).
    % CAVEAT declarado: velocidad*T sobreestima en ~7cm el desplazamiento
    % del tobillo realmente medido al cierre del ciclo (101.6 vs 94.5cm de
    % media) - probablemente por el desfase de la deteccion de eventos de
    % Zeni respecto del marcador de tobillo. Aun asi el error del modelo
    % baja, porque el error propio de la cadena era mayor (10.9cm).
...
% de hacer falta y el resultado mejora en las 4 metricas a la vez
% (X: r 0.997->0.998, RMSE 4.46->4.08cm; Y: r 0.953->0.985,
% RMSE 1.99->1.54cm). El modelo queda ademas con una sola correccion
% empirica de escala en vez de dos encadenadas.
...
fprintf('\n=== TOBILLO, MODELO FINAL (angulos calibrados LOSO + fases + rockers) vs Kuopio, N=%d ===\n', sum(ok2));
...
writetable(T, fullfile(carpeta, 'Evaluar_vs_Kuopio_Tobillo_Fases_resultados.csv'));
fprintf('Tabla: %s\n', fullfile(carpeta, 'Evaluar_vs_Kuopio_Tobillo_Fases_resultados.csv'));

if ~hacer_figura, return; end

% ---------------- Figura de grupo (PAREADA, sin promediar sujetos) ------
col_x = [0.85 0.33 0.10]; col_y = [0.20 0.45 0.70];
fig = figure('Name','TOBILLO modelo final (fases+rockers) vs Kuopio','Position',[40 20 1250 1050],'Color','w');
...
sgtitle(sprintf('TOBILLO vs Kuopio 2024 (overground, N=%d) - angulos calibrados (LOSO) + cadena por fases + rockers', sum(ok2)), 'FontWeight','bold');
out_png = fullfile(carpeta, 'Evaluar_vs_Kuopio_Tobillo_Fases_figura.png');

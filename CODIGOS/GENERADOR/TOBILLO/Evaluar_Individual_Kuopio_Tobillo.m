function T6 = Evaluar_Individual_Kuopio_Tobillo()
% EVALUAR_INDIVIDUAL_KUOPIO_TOBILLO  25-ago-2026: el MODELO FINAL de
%                   TOBILLO (fases + residuo de rockers + ganancia LOSO,
%                   ver Evaluar_vs_Kuopio_Tobillo_Fases.m) sujeto por
%                   sujeto, no en promedio.
%
...
%   distinta mezcla curvas que no son comparables entre si.
%
%   UNA SOLA IMPLEMENTACION DEL MODELO: este script NO recalcula nada -
%   llama a Evaluar_vs_Kuopio_Tobillo_Fases(false) y grafica las mismas
%   curvas que producen las estadisticas reportadas.
%
%   MISMOS 6 sujetos que RODILLA/ e INCLINACION_TIBIAL/ (el mas pesado 40,
%   el mas alto 37, el mas liviano 43, la mas baja 46 F, una mujer media
%   19, un hombre de talla media 28) - deliberadamente los mismos, para
%   poder comparar los 3 modelos sujeto a sujeto.
...
[~, D] = Evaluar_vs_Kuopio_Tobillo_Fases(false);
pct = D.pct;

fig = figure('Name','TOBILLO - comparacion individual, Kuopio', 'Position',[30 20 1400 1000], 'Color','w');
...
fprintf('\n=== TOBILLO, comparacion individual (6 sujetos de antropometria diversa) ===\n');
disp(T6);

sgtitle('TOBILLO - modelo final, sujeto por sujeto (cada uno con SU sexo/talla/masa y SU velocidad medida)', 'FontWeight','bold');

writetable(T6, fullfile(carpeta, 'Evaluar_Individual_Kuopio_Tobillo_resultados.csv'));
out_png = fullfile(carpeta, 'Evaluar_Individual_Kuopio_Tobillo_figura.png');
exportgraphics(fig, out_png, 'Resolution', 150);
fprintf('Figura guardada en: %s\n', out_png);
fprintf('Tabla guardada en: %s\n', fullfile(carpeta, 'Evaluar_Individual_Kuopio_Tobillo_resultados.csv'));

...
## Tarea 4 (INCLINACION_TIBIAL/Yun) — COMPLETADA (Agente D, cierre del checkpoint parcial de arriba)

- Leídos `PLAN_ZHAO_YUN.md`, `Evaluar_vs_Kuopio_AnguloTibial.m` y `Evaluar_Individual_Kuopio_AnguloTibial.m` (plantillas de Koopman) antes de escribir nada, además de `Evaluar_vs_Kuopio_AnguloTibial_Zhao.m` como ejemplo de adaptación ya terminada.
- Construidos `INCLINACION_TIBIAL/Evaluar_vs_Kuopio_AnguloTibial_Yun.m` y `INCLINACION_TIBIAL/Evaluar_Individual_Kuopio_AnguloTibial_Yun.m`, copia estructural exacta de los de Koopman. Única diferencia real del pipeline: la predicción cruda sale de `p14 = vector14_desde_antropometria(antro); Y = Yun2014_Wrapper(p14); t_Y = rad2deg(Y.theta_tibia_via_tobillo_R_rad);` en vez de la llamada a `Koopman2014_Core` — canal R_/vía tobillo, la regla ya corregida el 26-ago en `Obtener_Angulos_Candidato.m`/`Obtener_Theta_Tibia_Candidato.m` (NO vía rodilla). El helper `vector14_desde_antropometria` se copió igual que en `RODILLA/Evaluar_vs_Winter.m`/`Obtener_Theta_Tibia_Candidato.m`, no se inventó uno nuevo.
- **Nombres de archivo de salida verificados dos veces antes de correr** (lección explícita del bug de TOBILLO/Zhao mencionado en la tarea): las 4 salidas llevan sufijo `_Yun` (`Evaluar_vs_Kuopio_AnguloTibial_Yun_resultados.csv`/`_figura.png`, `Evaluar_Individual_Kuopio_AnguloTibial_Yun_resultados.csv`/`_figura.png`) — no se sobrescribió ningún resultado de Koopman ni de Zhao.
- Corridos ambos scripts en MATLAB real (`C:\Program Files\MATLAB\R2025b\bin\matlab.exe -batch`, con `run_in_background: true` por la advertencia de rendimiento de Yun). Grupo: 00:28–00:54 (~26 min). Individual: 00:54(lanzado)–01:20 (~26 min, recalcula los 15 sujetos de nuevo porque `Evaluar_Individual_Kuopio_AnguloTibial_Yun.m` llama a `Evaluar_vs_Kuopio_AnguloTibial_Yun(false)`, que no cachea entre invocaciones de MATLAB separadas — mismo comportamiento ya documentado para TOBILLO/Yun). N=15 en ambas corridas, sin fallos de sujeto, sin NaN en ninguna columna de r/RMSE/RMSEnorm de los 2 CSV.
- **Resultado de grupo (N=15, ángulo tibial nativo vía tobillo R_, Yun vs Kuopio 2024) — confirma y cierra el checkpoint parcial de arriba, cifras exactas del CSV:**
  - Sin calibrar: r=-0.300 (rango por sujeto -0.042 a -0.449, NEGATIVO en los 15), RMSE=22.43°, RMSEnorm=4.768 ("Deficiente").
  - Calibrado (afín LOSO): r=0.300 (misma magnitud, la calibración con ganancia negativa solo invierte el signo — no corrige la relación real), RMSE=16.66°, RMSEnorm=4.417 (sigue "Deficiente", <1 sería "Excelente"). Ganancia media=-0.742 (rango -0.691 a -0.860, negativa en los 15), offset medio=-20.89°.
- **Resultado individual (6 sujetos [40,37,43,46,19,28], igual que Koopman/Zhao):**

  | sub_id | sexo | talla_cm | masa_kg | r_ang (cal) | RMSE crudo (°) | RMSE cal (°) | RMSEnorm crudo | RMSEnorm cal |
  |---|---|---|---|---|---|---|---|---|
  | 40 | M | 183.6 | 136.14 | 0.435 | 24.67 | 17.39 | 5.99 | 5.86 |
  | 37 | M | 186.6 | 90.38 | 0.365 | 20.21 | 13.00 | 4.40 | 3.73 |
  | 43 | M | 179.0 | 61.02 | 0.238 | 25.32 | 20.36 | 5.23 | 5.43 |
  | 46 | F | 165.0 | 64.61 | 0.233 | 22.08 | 17.39 | 4.59 | 4.52 |
  | 19 | F | 169.0 | 63.24 | 0.042 | 19.40 | 15.49 | 4.23 | 4.41 |
  | 28 | M | 172.5 | 103.59 | 0.435 | 26.98 | 20.43 | 5.53 | 4.70 |

  Consistente con el promedio de grupo, sin outliers extremos — ningún sujeto se comporta radicalmente distinto (r_ang entre 0.042 y 0.435, todos "Deficiente" en RMSEnorm calibrado).
- **Interpretación, tal como pedía el plan (no forzar una corrección, reportar el hallazgo tal cual):** el ángulo tibial nativo de Yun (`theta_tibia_via_tobillo_R_rad`) NO reproduce la forma del ángulo tibial real de Kuopio con su canal nativo — la correlación cruda es negativa en los 15 sujetos, sin excepción. Es el mismo defecto de forma/fase ya encontrado para Zhao en esta misma pieza (Tarea 3: r crudo=-0.303) y para Yun en TOBILLO (Tarea 2: ganancia de tibia negativa en los 15 sujetos) — **tres piezas independientes (rodilla ya cerrada antes, tobillo y ángulo tibial esta noche) apuntan al mismo patrón: ni Zhao ni Yun son competitivos con Koopman para el segmento tibial con su configuración nativa**, mientras que Koopman (r=0.992 crudo, un solo defecto de escala que la calibración afín corrige del todo) sigue siendo claramente superior para esta pieza. Igual que en Zhao, la ganancia de calibración sale NEGATIVA (media -0.742, rango -0.691 a -0.860) — la calibración afín solo invierte el signo del error de fase, no lo corrige; RMSEnorm calibrado se queda en "Deficiente" (4.417) en vez de bajar a "Excelente" como pasa con Koopman (0.92). El "truco de lado"/vía rodilla no se probó — decisión D2 de `DECISIONES.md`, explícitamente no reabierta por esta tarea.
- No se tocó ningún archivo de Koopman (`Evaluar_vs_Kuopio_AnguloTibial.m`, `Evaluar_Individual_Kuopio_AnguloTibial.m`) ni los de Zhao (`..._Zhao.m`), ni se re-ejecutaron.
- Archivos generados: `INCLINACION_TIBIAL/Evaluar_vs_Kuopio_AnguloTibial_Yun.m`, `INCLINACION_TIBIAL/Evaluar_vs_Kuopio_AnguloTibial_Yun_resultados.csv`, `INCLINACION_TIBIAL/Evaluar_vs_Kuopio_AnguloTibial_Yun_figura.png`, `INCLINACION_TIBIAL/Evaluar_Individual_Kuopio_AnguloTibial_Yun.m`, `INCLINACION_TIBIAL/Evaluar_Individual_Kuopio_AnguloTibial_Yun_resultados.csv`, `INCLINACION_TIBIAL/Evaluar_Individual_Kuopio_AnguloTibial_Yun_figura.png` — los 6 en `CODIGOS/GENERADOR/INCLINACION_TIBIAL/`. Los logs de consola `yun_grupo_log.txt`/`yun_individual_log.txt` se dejan en disco sin commitear (mismo criterio que el resto de la noche: los logs de corrida no se versionan).
- Checkbox de Tarea 4 marcado en `PLAN_ZHAO_YUN.md`.

## Tareas 2 y 4 (TOBILLO/Yun, INCLINACION_TIBIAL/Yun) — CERRADAS
- Ambos scripts individuales terminaron sin proceso MATLAB colgado (confirmado con `tasklist`, ya no aparece ninguno).
- Verificados: 6/6 sujetos esperados `[40,37,43,46,19,28]`, sin NaN, valores consistentes con el resultado de grupo ya registrado (ej. sujeto 40 r_x=0.9857 coincide en ambas tablas). PNG de ambos con tamaño razonable (298KB y 335KB), no corruptos.
- **Tarea 2 (TOBILLO/Yun) resultado final:** r_x individual entre 0.978-0.996, r_y entre 0.736-0.907 — consistente con el promedio de grupo (r_x=0.989, r_y=0.831), ningún sujeto extremo. Veredicto final: Yun no competitivo con Koopman (X RMSE 7.08cm vs 2.90cm; Y r 0.831 vs 0.985).
- **Tarea 4 (INCLINACION_TIBIAL/Yun) resultado final:** r_ang individual entre 0.04-0.44 (todos positivos tras calibración, pero bajos), RMSEnorm calibrado entre 3.7-5.9 — todos en "Deficiente" en la escala del proyecto. Veredicto final: Yun no viable para ángulo tibial, mismo problema de fase que Zhao.
- Checkboxes de Tarea 2 y Tarea 4 marcados `[x]` en `PLAN_ZHAO_YUN.md`.
- Las 4 Tareas de Fase 2 (A, B, C, D) quedan COMPLETAS. Se pasa a Fase 3 (validación y documentación).

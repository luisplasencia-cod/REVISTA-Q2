function [T, D] = Evaluar_vs_Kuopio_AnguloTibial_Yun(hacer_figura)
% EVALUAR_VS_KUOPIO_ANGULOTIBIAL_YUN  27-ago-2026: replica EXACTA de
%                   Evaluar_vs_Kuopio_AnguloTibial.m (Koopman) pero con
%                   Yun 2014 como candidato. Ver PLAN_ZHAO_YUN.md,
%                   seccion INCLINACION_TIBIAL (Tarea 4), y DECISIONES.md
%                   D2 - se usa el canal NATIVO/default ya establecido en
%                   el proyecto (theta_tibia_via_tobillo_R_rad, corregido
%                   hoy en Obtener_Angulos_Candidato.m/
%                   Obtener_Theta_Tibia_Candidato.m para NO usar via
%                   rodilla), SIN mezclar canales ni inventar ajustes.
%                   Si el resultado sale con r bajo por el defasaje ya
%                   documentado para Yun (RODILLA/CIERRE_RODILLA.md), es
%                   el resultado esperado y se reporta tal cual.
%
%   MODELO EVALUADO: Yun 2014 (angulo tibial nativo via tobillo,
%   theta_tibia_via_tobillo_R_rad, salida directa del toolbox de
%   terceros) + CALIBRACION AFIN LOSO (mismo procedimiento que Koopman y
%   Zhao, para que la comparacion entre candidatos sea igual de justa -
%   la calibracion no cambia r, solo escala/offset).
%
...
%     Yun:     Y = Yun2014_Wrapper(vector14_desde_antropometria(antro));
%              t_Y = rad2deg(Y.theta_tibia_via_tobillo_R_rad);
%              (vector de 14 parametros del toolbox de terceros, mismo
%              helper local ya usado en Obtener_Angulos_Candidato.m /
%              RODILLA/Evaluar_vs_Winter.m - NO se inventa uno nuevo.
%              Canal R_/via tobillo, NO via rodilla - regla ya
%              establecida y corregida hoy mismo, D2 de DECISIONES.md).
...
%   archivos por cada llamada a sujeto - este script es sustancialmente
%   mas lento que el de Koopman/Zhao para N=15 sujetos.
%
%   SALIDAS
%     T : tabla por sujeto. D : struct con las curvas por sujeto - lo
%     consume Evaluar_Individual_Kuopio_AnguloTibial_Yun.m para que
...
% --- Paso 1: angulo real y prediccion cruda (Yun, via tobillo R_) ---
for i = 1:n
    sid = ids(i);
    try
        ...
        t_Y = rad2deg(Y.theta_tibia_via_tobillo_R_rad);
        pct_Y = linspace(0,100,numel(t_Y));
        theta_pred_deg = interp1(pct_Y, t_Y, pct, 'pchip');
...
fprintf('\n=== ANGULO TIBIAL (YUN 2014) vs Kuopio 2024, N=%d ===\n', sum(ok));
...
writetable(T, fullfile(carpeta, 'Evaluar_vs_Kuopio_AnguloTibial_Yun_resultados.csv'));
fprintf('Tabla: %s\n', fullfile(carpeta, 'Evaluar_vs_Kuopio_AnguloTibial_Yun_resultados.csv'));
...
fig = figure('Name','Angulo tibial (Yun 2014) vs Kuopio 2024','Position',[40 20 1250 1050],'Color','w');
...
legend({'real (por sujeto)','Yun crudo (por sujeto)'},'Location','northwest');
...
legend({'real (por sujeto)','Yun calibrado (por sujeto)'},'Location','northwest');
...
sgtitle(sprintf('ANGULO TIBIAL (YUN 2014) vs Kuopio 2024 (overground, N=%d) - crudo vs calibrado (afin, LOSO)', sum(ok)), 'FontWeight','bold');
out_png = fullfile(carpeta, 'Evaluar_vs_Kuopio_AnguloTibial_Yun_figura.png');
...
% Identico al helper local ya duplicado en Obtener_Theta_Tibia_Candidato.m /
% Obtener_Angulos_Candidato.m / RODILLA/Evaluar_vs_Winter.m / etc. - se
% copia igual, no se inventa uno nuevo (instruccion explicita del plan).

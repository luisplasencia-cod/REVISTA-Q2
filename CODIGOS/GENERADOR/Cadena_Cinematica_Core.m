...
% no supuesto): X sale directo de la convencion ya validada de
% Segmento_Posicion_Core.m SIN necesitar ajuste - se comparo la
% correlacion angulo-vs-X del generador contra Control_apoyo_Luis_V4.csv
% real (95 filas validas, se excluyo una fila final con NaN del propio
% archivo) y coincidio de entrada: corr(ang,X) real=-0.993, generado=
% -1.000, mismo signo. Y SI necesito un cambio de signo (mismo metodo):
% corr(ang,Y) real=+0.529 vs. generado=-0.450 ANTES de ese fix - signo
% opuesto, corregido invirtiendo solo el eje Y (no el X). Un reflejo
% sobre el eje X preserva distancias, por eso no rompio ninguna prueba
% de distancia ya validada, y esta generalizacion a "cualquier punto del
% segmento" hereda el mismo signo ya verificado (es la misma direccion
% angular, solo cambia cuanto se avanza por ella).
%
